...
class MatchingHandler extends QuestionHandler {
    ...
    getAnswer() {
        const answers = Array.from(this.pairsContainer.querySelectorAll('select')).map(s => s.value);

        // Return null if all selections are empty
        return answers.every(a => a === '') ? null : answers;
    }
}
...
class MultiSelectHandler extends QuestionHandler {
    ...
    getAnswer() {
        const checkedOptions = Array.from(this.optionsContainer.querySelectorAll('input[type="checkbox"]:checked')).map(c => c.value);
        // Return null if no options are selected
        return checkedOptions.length === 0 ? null : checkedOptions;
    }
}
...
class OrderingHandler extends QuestionHandler {
    ...
    getAnswer() {
        const currentOrder = Array.from(this.itemsContainer.querySelectorAll('.ordering-item')).map(i => i.getAttribute('data-id'));
        const initialOrder = this.initialOrder || currentOrder; // Store initial order on first render
        
        // Return null if order hasn't changed from initial state
        return JSON.stringify(currentOrder) === JSON.stringify(initialOrder) ? null : currentOrder;
    }
}
...
class QuizState {
    ...
    saveCurrentAnswer(answer) {
        ...
        const processedAnswer = (typeof answer === 'string' && answer.trim() === '') ? null : answer;

        const question = this.questions[this.currentQuestionIndex];
        this.userAnswers[this.currentQuestionIndex].answer = processedAnswer;
        this.userAnswers[this.currentQuestionIndex].isCorrect = this.checkAnswerCorrectness(question, processedAnswer);
    }

    checkAnswerCorrectness(question, answer) {
        // First check for empty answer
        if (answer === null || 
            answer === undefined || 
