class CentralBlockPracticeText extends HTMLElement {
  constructor() {
    super();
    this._config = {};
  }
  ...
  render() {
    ...
    this.innerHTML = `
      <div class="fibmgt-container">
        <h3 class="fibmgt-question">${question}</h3>
        <div class="fibmgt-board">
          <svg class="fibmgt-lines"></svg>
          <div class="fibmgt-center">${center_label}</div>
          ${blocks
            .map(
              (b, i) => `
            <div class="fibmgt-block" data-index="${i}">
              <div class="fibmgt-label">${b.a} ${b.b}</div>
              <input class="fibmgt-input" type="text" />
            </div>
          `
            )
            .join('')}
  ...
  _positionBlocks() {
    const board = this.querySelector('.fibmgt-board');
    const center = this.querySelector('.fibmgt-center');
    const blocks = this.querySelectorAll('.fibmgt-block');
    const svg = this.querySelector('.fibmgt-lines');
  ...
  getUserAnswer() {
    ...
    const blocks = this.querySelectorAll('.fibmgt-block');
    blocks.forEach((block, i) => {
      const input = block.querySelector('.fibmgt-input').value.trim();
...
customElements.define('fill-in-blank-multi-graph-text', CentralBlockPracticeText);

/*



<link rel="stylesheet" href="../styles/fill-in-blank-multi-graph-text.css" />

<fill-in-blank-multi-graph-text id="practice"></fill-in-blank-multi-graph-text>

<script src="../scripts/components/fill-in-blank-multi-graph-text.js"></script>
<script src="../scripts/utils/quiz_evaluators/FillInTheBlankMutiGraph.js"></script>

  <script>
    const config = {
      type: "fill_in_blank_multi_graph_text",
      id: "001",
