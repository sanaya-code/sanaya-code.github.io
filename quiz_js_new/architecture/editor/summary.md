...
- `EditorPanelController` — loadQuestion, loadNewQuestion, showPreview, showTypeSelector, clear. Also owns internal tab-switched logic.
- `ImportPanelController` — show(questions), hide(), getSelectedQuestions(), signal bindings
...
- `TopbarComponent` — wraps `#topbar`, exposes signal binding methods
- `EmptyStateComponent` — wraps `#empty-state`, exposes show/hide/error/draft methods
- `QuestionListComponent` — custom element `question-list`, renders card list, emits question-selected / question-deleted / question-reordered
- `EditorPanelComponent` — custom element `editor-panel`, renders form+preview shell, emits question-saved / question-closed / type-selected / type-selector-closed / tab-switched
- `TypeSelectorComponent` — custom element `type-selector`, rendered inside editor-panel
- `ImportPanelComponent` — wraps `#import-panel` overlay, renders question cards for selection, emits import-confirmed / import-cancelled
...
- `TopbarHandler` — _onNew, _onLoadJson, _onExportJson, _onAddQuestion, _onImportJson
- `ImportPanelHandler` — openWithQuestions(questions), handles import-confirmed / import-cancelled
...
CSS generation rule: Run sed on MCQ `style.css` as the base, then append only
new rules for the unique answer widget. Never write the base from scratch.
...
## Directory Tree
    ...
    editor/
      ...
      controller.js                       MainController — orchestrator
      editor_form_registry.js             registry: maps type string to form tag + default data
      style.css                           global shell styles, CSS variables, layout

      state/
        index.js                          StateController: delegates to 3 sub-states
        question_state.js                 owns questions array (add/delete/reorder/save/import)
      ...
      components/
        topbar/
          ...
          controller.js                   TopbarController: add/load/export/import/filePicker ops
        empty_state/
        ...
        preview_panel/
          ...
          style.css
        import_panel/
          component.js                    ImportPanelComponent: overlay, question card selection
          controller.js                   ImportPanelController: show/hide/getSelected
          style.css

        question_forms/
          mcq_form/                       component.js, style.css — TEMPLATE for all forms
      ...
      event_handler/
        ...
        topbar/
          handler.js                      TopbarHandler: onNew/Load/Export/AddQuestion/ImportJson
        import_panel/
          handler.js                      ImportPanelHandler: openWithQuestions, confirm/cancel
...
All 23 question type forms are built. Nothing pending.
